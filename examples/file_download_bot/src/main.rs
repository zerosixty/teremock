...
#[cfg(test)]
mod tests {
    use teremock::{MockBot, MockMessageDocument, MockMessageText};
    ...
    async fn test_not_a_document() {
        ...
        assert_eq!(last_msg.text(), Some("Not a document"));
    }

    #[tokio::test]
    async fn test_download_document_and_check() {
        let mut bot = MockBot::new(MockMessageDocument::new(), handler_tree()).await;
        bot.dispatch().await;
        let responses = bot.get_responses();
        let last_msg = responses.sent_messages.last().expect("No messages sent");
        assert_eq!(last_msg.text(), Some("Downloaded!"));
    }
}
