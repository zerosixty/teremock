...
impl BodyChatId {
    ...
    ///
    /// For text usernames, returns a default placeholder ID since username
    /// resolution isn't implemented. See [`common::DEFAULT_TEXT_CHAT_ID`].
    pub fn id(&self) -> i64 {
        match self {
            BodyChatId::Text(_) => common::DEFAULT_TEXT_CHAT_ID,
            BodyChatId::Id(id) => *id,
        }
    }
...
pub(crate) struct BotApiError {
    error: ApiError,
}

impl BotApiError {
    pub(crate) fn new(error: ApiError) -> Self {
        Self { error }
    }
}
...
/// Macro to check if a message exists and return an appropriate Telegram API error if not.
///
/// Uses `ApiError::MessageToReplyNotFound` which is the closest match for referencing
/// a non-existent message in reply_parameters context.
///
/// For routes returning `RouteResult`, use the `result` variant.
/// For routes returning `impl Responder`, use the default variant.
macro_rules! check_if_message_exists {
    // Variant for RouteResult return type (Result<HttpResponse, RouteError>)
    ($lock:expr, $msg_id:expr, result) => {
        if $lock.messages.get_message($msg_id).is_none() {
            use actix_web::ResponseError as _;
            return Err($crate::server::routes::common::RouteError::new(
                $crate::server::routes::BotApiError::new(
                    teloxide::ApiError::MessageToReplyNotFound,
                )
                .error_response(),
            ));
        }
    };
    // Default variant for impl Responder return type
    ($lock:expr, $msg_id:expr) => {
        if $lock.messages.get_message($msg_id).is_none() {
            use actix_web::ResponseError as _;
            return $crate::server::routes::BotApiError::new(
                teloxide::ApiError::MessageToReplyNotFound,
            )
            .error_response();
        }
