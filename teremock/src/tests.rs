...
//
// Dialogue state tests (black-box style - no direct state manipulation)
//
...
// Test that dialogue naturally transitions through states (black-box testing)
#[tokio::test]
async fn test_dialogue_state_transition() {
    let storage = InMemStorage::<State>::new();

    // First message - starts in Start state, transitions to NotStart
    let mut bot = MockBot::new(MockMessageText::new().text("test"), get_dialogue_schema()).await;
    ...
    // Second message - should now be in NotStart state
    bot.update(MockMessageText::new().text("another"));
    bot.dispatch().await;
...
#[tokio::test]
async fn test_echo() {
    let mut bot = MockBot::new(MockMessageText::new().text("/echo echo"), get_schema()).await;
...
async fn test_panic() {
    ...
    let bot = MockBot::new(MockMessageText::new().text("/panic"), get_schema()).await;
...
async fn test_error_handler() {
    let mut bot = MockBot::new(MockMessageText::new().text("/panic"), get_schema()).await;
    ...
    // Error should mention the message reply issue - we now return proper teloxide ApiError
    assert!(errors[0].contains("message") && errors[0].to_lowercase().contains("not found"));
}

#[tokio::test]
async fn test_no_updates() {
    let empty: Vec<MockMessageDice> = vec![];
    let mut bot = MockBot::new(empty, get_schema()).await;
...
async fn test_send_photo() {
    let mut bot = MockBot::new(MockMessageText::new().text("/photo"), get_schema()).await;
...
async fn test_send_video() {
    let mut bot = MockBot::new(MockMessageText::new().text("/video"), get_schema()).await;
...
async fn test_send_audio() {
    let mut bot = MockBot::new(MockMessageText::new().text("/audio"), get_schema()).await;
...
async fn test_send_voice() {
    let mut bot = MockBot::new(MockMessageText::new().text("/voice"), get_schema()).await;
...
async fn test_send_video_note() {
    let mut bot = MockBot::new(MockMessageText::new().text("/videonote"), get_schema()).await;
...
async fn test_send_document() {
    let mut bot = MockBot::new(MockMessageText::new().text("/document"), get_schema()).await;
...
async fn test_send_animation() {
    let mut bot = MockBot::new(MockMessageText::new().text("/animation"), get_schema()).await;
...
async fn test_send_media_group() {
    let mut bot = MockBot::new(MockMessageText::new().text("/mediagroup"), get_schema()).await;
...
async fn test_send_location() {
    let mut bot = MockBot::new(MockMessageText::new().text("/location"), get_schema()).await;
...
async fn test_send_venue() {
    let mut bot = MockBot::new(MockMessageText::new().text("/venue"), get_schema()).await;
...
async fn test_send_contact() {
    let mut bot = MockBot::new(MockMessageText::new().text("/contact"), get_schema()).await;
...
async fn test_send_dice() {
    let mut bot = MockBot::new(MockMessageText::new().text("/dice"), get_schema()).await;
...
async fn test_send_poll() {
    let mut bot = MockBot::new(MockMessageText::new().text("/poll"), get_schema()).await;
...
async fn test_send_sticker() {
    let mut bot = MockBot::new(MockMessageText::new().text("/sticker"), get_schema()).await;
...
async fn test_edit_message() {
    let mut bot = MockBot::new(MockMessageText::new().text("/edit"), get_schema()).await;
...
async fn test_edit_message_unchanged() {
    let mut bot = MockBot::new(MockMessageText::new().text("/editunchanged"), get_schema()).await;
    let error_handler = Arc::new(MyErrorHandler::new());
    bot.error_handler(error_handler.clone());
...
async fn test_edit_caption() {
    let mut bot = MockBot::new(MockMessageText::new().text("/editcaption"), get_schema()).await;
...
async fn test_delete_message() {
    let mut bot = MockBot::new(MockMessageText::new().text("/delete"), get_schema()).await;
...
async fn test_delete_messages() {
    let mut bot = MockBot::new(MockMessageText::new().text("/deletebatch"), get_schema()).await;
...
async fn test_answer_callback_query() {
    let mut bot = MockBot::new(MockCallbackQuery::new().data("test"), get_schema()).await;
...
async fn test_pin_message() {
    let mut bot = MockBot::new(MockMessageText::new().text("/pinmessage"), get_schema()).await;
...
async fn test_forward_message() {
    let mut bot = MockBot::new(MockMessageText::new().text("/forwardmessage"), get_schema()).await;
...
async fn test_copy_message() {
    let mut bot = MockBot::new(MockMessageText::new().text("/copymessage"), get_schema()).await;
...
async fn test_ban_and_unban() {
    let mut bot = MockBot::new(MockMessageText::new().text("/ban"), get_schema()).await;
...
async fn test_restrict() {
    let mut bot = MockBot::new(MockMessageText::new().text("/restrict"), get_schema()).await;
...
async fn test_send_chat_action() {
    let mut bot = MockBot::new(MockMessageText::new().text("/chataction"), get_schema()).await;
...
async fn test_set_my_commands() {
    let mut bot = MockBot::new(MockMessageText::new().text("/setmycommands"), get_schema()).await;
...
async fn test_send_invoice() {
    let mut bot = MockBot::new(MockMessageText::new().text("/invoice"), get_schema()).await;
...
async fn test_edited_message() {
    ...
    let mut bot = MockBot::new(mock_message.clone(), get_schema()).await;
    bot.dispatch().await;
...
// Test multi-dispatch with persistent server (teremock's main feature)
#[tokio::test]
async fn test_multi_dispatch_persistent_server() {
    let mut bot = MockBot::new(MockMessageText::new().text("/echo first"), get_schema()).await;

    // First dispatch
    bot.dispatch().await;
    let first_response = bot.get_responses().sent_messages.pop().unwrap();
    assert_eq!(first_response.text(), Some("/echo first"));

    // Second dispatch - same server instance
    bot.update(MockMessageText::new().text("/echo second"));
    bot.dispatch().await;
    let second_response = bot.get_responses().sent_messages.pop().unwrap();
    assert_eq!(second_response.text(), Some("/echo second"));

    // Third dispatch - still the same server
    bot.update(MockMessageText::new().text("/echo third"));
    bot.dispatch().await;
    let third_response = bot.get_responses().sent_messages.pop().unwrap();
    assert_eq!(third_response.text(), Some("/echo third"));
}
